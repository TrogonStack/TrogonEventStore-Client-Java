...
/**
 * Represents a TrogonEventStore client for stream operations. A client instance maintains two-way communication with TrogonEventStore.
 * Many threads can use the TrogonEventStore client simultaneously, or a single thread can make many asynchronous requests.
 */
public class TrogonEventStoreClient extends TrogonEventStoreClientBase {
    private TrogonEventStoreClient(TrogonEventStoreClientSettings settings) {
        super(settings);
    }

    /**
     * Creates a gRPC client to TrogonEventStore database.
     */
    public static TrogonEventStoreClient create(TrogonEventStoreClientSettings settings) {
        return new TrogonEventStoreClient(settings);
    }
